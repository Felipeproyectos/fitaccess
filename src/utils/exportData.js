...
export function exportXLSX(filename, headers, rows) {
  ...
  // Style header row (SheetJS CE supports limited styling via s property)
  headers.forEach((_, i) => {
    const cell = XLSX.utils.encode_cell({ r: 0, c: i });
    if (ws[cell]) {
      ws[cell].s = {
        font: { bold: true, color: { rgb: "FFFFFF" } },
        fill: { fgColor: { rgb: "FF3B3B" } },
        alignment: { horizontal: "center" },
        border: {
          bottom: { style: "medium", color: { rgb: "FF3B3B" } }
        }
      };
    }
  });
...
// ---- XLSX Template (empty with headers) ----
export function exportXLSXTemplate(filename, headers) {
  exportXLSX(filename, headers, []);
}

// ---- CSV (legacy, kept for other uses) ----
